...
def parse(file_content):
    """
    Parses the uploaded CSV file, capturing all financial columns provided
    ...
    try:
        ...
        if not all(col in df.columns for col in required_cols.values()):
            raise ValueError("CSV file is missing required columns. Please ensure it contains: 'Instrument', 'Qty', 'Avg. cost', 'Invested', 'Cur. val', and 'P&L'.")
