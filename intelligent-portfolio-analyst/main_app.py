...
def setup_tabs(data):
    """Creates the final, multi-tab layout using stateful st.tabs."""
...
def display_what_if_analysis(original_stocks):
    """
    Displays the 'What If?' analysis tab with a polished UI, including improved
    input section and a more compact, reordered results display.
    """
    ...
    with st.form("what_if_form"):
        st.subheader("Select a stock, enter the quantity, then click 'Analyze Scenario' to fetch the price and see the results.")
        col1, col2 = st.columns(2)
        with col1:
        ...
        submit_button = st.form_submit_button("Analyze Scenario", use_container_width=True)

    # The analysis logic now only runs AFTER the button is pressed
    if submit_button and selected_ticker:
        with st.spinner(f"Fetching price and analyzing scenario for {selected_ticker}..."):
            price_history = data_fetchers.get_price_history(f"{selected_ticker}.NS", period="1d")
            
            if not price_history:
                st.error(f"Could not fetch price for {selected_ticker}. Please try another stock.")
                return
            ...
            # --- Reordered and Polished Results UI ---
            st.subheader("Comparative Sector Allocation")
            col1, col2, col3 = st.columns(3)
            with col1:
                st.markdown("###### Ideal Allocation")
                ideal_df = pd.DataFrame({'Sector': ['Financials', 'IT', 'Consumer Goods', 'Healthcare', 'Energy', 'Industrials', 'Materials', 'Utilities'], 'Value': [25, 15, 15, 10, 10, 10, 5, 10]})
                fig_ideal = px.pie(ideal_df, names='Sector', values='Value', hole=0.4, height=250)
                fig_ideal.update_layout(showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
                st.plotly_chart(fig_ideal, use_container_width=True)
            with col2:
                st.markdown("###### Your Original Allocation")
                ...
                if original_sector_allocation:
                    ...
                    fig_orig = px.pie(sector_df_orig, names='Sector', values='Value', hole=0.4, height=250)
                    fig_orig.update_layout(showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
                    st.plotly_chart(fig_orig, use_container_width=True)
            with col3:
                st.markdown("###### New Allocation (Simulated)")
                ...
                if new_sector_allocation:
                    ...
                    fig_new = px.pie(sector_df_new, names='Sector', values='Value', hole=0.4, height=250)
                    fig_new.update_layout(showlegend=False, margin=dict(l=10, r=10, t=10, b=10))
                    st.plotly_chart(fig_new, use_container_width=True)
            
            st.subheader("Analysis Results")
            with st.container(border=True):
                st.markdown("##### Trade & Portfolio Impact")
                col1, col2, col3, col4 = st.columns(4)
                col1.metric("Purchase Price", f"Rs.{purchase_price:,.2f}")
                col2.metric("Invested Amount", f"Rs.{invested_amount:,.2f}")
                col3.metric("New Portfolio Value", f"Rs.{new_metrics.get('current_value', 0):,.2f}")
...
def display_screener():
    """ Displays the Personalized Stock Screener tab. """
    st.header("Personalized Stock Screener")
    st.write("Find new investment ideas that match your personal style.")
    ...
    with st.form("screener_form"):
        ...
        ticker_list = st.text_area("Enter tickers to screen (comma-separated)", default_tickers)
...
def main():
    ...
    st.markdown("""
        <style>
            /* Make dataframes wrap text */
            .stDataFrame [data-col="3"] div {
                white-space: normal !important;
                word-wrap: break-word !important;
            }
            /* Reduce font size of metric values and labels */
    ...
    if uploaded_file:
        ...
        status_map = {
            "ingest_portfolio": "Reading and parsing your portfolio file...",
            "validate_ingestion": "Validating the format of your portfolio data...",
            "enrich_stocks": "Fetching live market data and performing AI analysis on each stock...",
            "news_analyzer": "Gathering and summarizing the latest news for your holdings...",
            "report_generator": "Compiling the final analysis report..."
        }
    ...
    st.divider()
    st.warning("""**Disclaimer** Investment in securities market are subject to market risks, read all the related documents carefully before investing. This is AI generated content and AI can make mistakes and this is for educational purpose only, and all the API's used in this app is taken from yfinance.""")
